...
(function($, undefined ) {	
  $.widget( "mobile.datebox", $.mobile.widget, {
	options: {
		theme: 'c',
		pickPageTheme: 'b', /*확인 버튼 및 바탕 화면 이구나 */
		pickPageInputTheme: 'c', /*input*/
		pickPageButtonTheme: 'a', /*button*/
		...
		headerFormat: 'YYYY년 mmm dd일 ddd',		
...
;					if ( $(this).data('datebox').options.mode === 'timebox' || $(this).data('datebox').options.mode === 'durationbox' ) {
	...
	_update: function() {
		...
		if ( o.mode === 'calbox' ) { // Meat and potatos - make the calendar grid.
			self.controlsInput.text( self.theDate.getFullYear()+"년 "+ o.monthsOfYear[self.theDate.getMonth()] );
	...
	_buildPage: function () {
		var self = this,
			...
			o.pickPageTheme = o.page;
	
		    if(o.button=="undefined"|| o.button==null){
		    	o.pickPageButtonTheme="b";}
		    else {
		    	o.pickPageButtonTheme = o.button; }
		...
		if ( o.mode === 'durationbox' ) {
			...
			if ( o.noSetButton === false ) {
				$("<a href='#'>" + o.setDurationButtonLabel + "</a>")
					.appendTo(controlsSet).buttonMarkup({theme: o.pickPageButtonTheme, icon: 'check', iconpos: 'left', corners:true, shadow:true})
					.bind('vclick', function(e) {
						e.preventDefault();
